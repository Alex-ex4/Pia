...
void registrarZona(Zona **zonas, int *cont) {
    ...
    do{
        valido=1;
        nuevaZona.id = leer_entero_validado("\nIngrese ID de la zona (1-9999): ", 1, 9999);
        for(int i=0;i<*cont;i++){
            if((*zonas)[i].id==nuevaZona.id){
                printf(ANSI_COLOR_RED "Error: El ID ya existe. Intente con otro.\n" ANSI_COLOR_RESET);
                valido=0;
            }
        }
    }while(valido==0);
...
void activarVent(Zona **zonas, int *cont){
    ...
    leer_cadena_validada("\nIngresa el nombre de la zona: ", nombreZona, sizeof(nombreZona), 1, 49);
...
void historial_por_zona(Zona **zonas, int *cont) {
    ...
    id_zona = leer_entero_validado("\nIngrese el ID de la zona para ver su historial (1-9999): ", 1, 9999);
...
void simular_monitoreo_tiempo_real(Zona **zonas, int *cont) {
    ...
    id_zona = leer_entero_validado("\nIngrese el ID de la zona para simular monitoreo (1-9999): ", 1, 9999);
...
void buscar_eventos_rango(Zona **zonas, int *cont) {
    ...
    id_zona = leer_entero_validado("\nIngrese el ID de la zona para buscar eventos (1-9999): ", 1, 9999);
...
void restaurar_configuracion_default(Zona **zonas, int *cont) {
    ...
    leer_cadena_validada("\nIngrese el nombre de la zona a restaurar: ", nombre, sizeof(nombre), 1, 49);
    ...
    printf("¿Desea restablecer el historial de eventos? (1 - Si/0 - No) \n");
    opcion = leer_entero_validado("Seleccion: ", 0, 1);
    if(opcion==0){
